-- Migration: Fix create_new_user_by_admin - remove confirmed_at from INSERT
-- confirmed_at is a GENERATED ALWAYS column in auth.users (computed from email_confirmed_at
-- and phone_confirmed_at), so it cannot be inserted directly.
...
BEGIN
  ...
  --       and must NOT be included in the INSERT statement.
  ...
  -- 4. The handle_new_user trigger on auth.users will automatically execute here.
  -- The trigger inserts a profile and a new organization.
  -- Find the new organization created by the trigger for this user.
