-- Migration: Force password change on first login + admin action to require it.
--
-- Adds a must_change_password flag on profiles. New users created via
-- create_new_user_by_admin start with it set to true (their initial password
-- was chosen by the admin, not by them). Existing users are unaffected
-- (default false), so nobody gets unexpectedly locked out.
--
-- Also adds force_password_change(user_id), an RPC that lets an org
-- admin/gerente flip the flag back on for an existing teammate — used by the
-- "Forçar troca de senha" action in the Usuários screen. This avoids adding
-- any new RLS policy to profiles (a recent policy change there caused a
-- login regression), keeping profiles' RLS exactly as it was before.
...
-- Recreate create_new_user_by_admin so freshly created users always start
